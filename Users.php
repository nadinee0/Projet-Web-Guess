...
class Users{
  ...
  // Date formate Method
   public function formatDate($date){
     // date_default_timezone_set('tunis/sfax');
  ...
  public function checkExistEmail($email){
    ...
    $stmt = $this->db->pdo->prepare($sql); //statment
    $stmt->bindValue(':email', $email); // lier un valeur a un parametre 
     $stmt->execute();
    if ($stmt->rowCount()> 0) {  // rowcount retourne le nombre de lignes affectées par la dernière requête
      return true;
    }else{
      return false;
    }
...
<strong>Error !</strong> Please, User Registration field must not be Empty !</div>';
        return $msg;
    }elseif (strlen($username) < 3) { // renvoie la longueur d'une chaîne
      $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Username is too short, at least 3 Characters !</div>';
        return $msg;
    }elseif (filter_var($mobile,FILTER_SANITIZE_NUMBER_INT) == FALSE) { //Supprime tous les caractères sauf les chiffres, et les signes plus et moins.
      $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
...
<strong>Error !</strong> Your Password Must Contain At Least 1 Number !</div>';
        return $msg;
    }elseif (filter_var($email, FILTER_VALIDATE_EMAIL === FALSE)) { //Supprime tous les caractères sauf les lettres, chiffres, et !#$%&'*+-=?^_`{|}~@.[]
      $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
